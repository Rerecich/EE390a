...
TFXP biases[7];  //that's what it is in cnnSolver?
...
void Conv2D_SW(TFXP *input, TFXP * output, TFXP * filters, TFXP * biases,
	  ...
	  ap_uint<32> convWidth = 3, ap_uint<32> convHeight = 3, ap_uint<1> apply_relu)
{
...
int main(int argc, char ** argv)
{
  ...
  srand(time(NULL));
  InitVectors(input, MAX_WIDTH * MAX_HEIGHT * MAX_CHANNELS, coeffs, MAX_CHANNELS * MAX_FILTERS * 9, biases, 7);
