...
# 10 "/scrap/users/Xilinx2022.2/Vitis_HLS/2022.2/common/technology/autopilot/ap_int.h"
...
# 11 "/scrap/users/Xilinx2022.2/Vitis_HLS/2022.2/common/technology/autopilot/ap_int.h" 2
...
# 10 "/scrap/users/Xilinx2022.2/Vitis_HLS/2022.2/common/technology/autopilot/ap_fixed.h"
...
# 361 "/scrap/users/Xilinx2022.2/Vitis_HLS/2022.2/common/technology/autopilot/ap_int.h" 2
...
#pragma HLS INTERFACE s_axilite port=apply_relu
 ...
 VITIS_LOOP_38_1: for (uint32_t iFilter = 0; iFilter < numFilters; ++ iFilter) {
  TFXP coeff_cache[256][3][3];
#pragma HLS ARRAY_PARTITION variable=coeff_cache complete dim=3


 VITIS_LOOP_43_2: for (uint32_t iChannel = 0; iChannel < numChannels; ++iChannel) {
      VITIS_LOOP_44_3: for (uint32_t cy = 0; cy < convHeight; ++cy) {
         VITIS_LOOP_45_4: for (uint32_t cx = 0; cx < convWidth; ++cx) {
     ...
     VITIS_LOOP_58_5: for (uint32_t y = 0; y < (inputHeight-2); ++y) {
      TFXP row_buffer[3][4064];
#pragma HLS ARRAY_PARTITION variable=row_buffer complete dim=1


 VITIS_LOOP_63_6: for (uint32_t cy = 0; cy < convHeight; ++cy) {
       uint32_t iy = y + cy;

       VITIS_LOOP_66_7: for (uint32_t iChannel = 0; iChannel < numChannels; ++iChannel) {

        VITIS_LOOP_68_8: for (uint32_t x = 0; x < inputWidth; ++x) {
#pragma HLS PIPELINE II=1
 row_buffer[cy][iChannel*inputWidth + x] = *(input + iChannel*inputWidth*inputHeight + iy*inputWidth + x);
      ...
      VITIS_LOOP_76_9: for (uint32_t x = 0; x < (inputWidth-2); ++ x) {
       TFXP acc = 0;


       VITIS_LOOP_80_10: for (uint32_t iChannel = 0; iChannel < numChannels; ++ iChannel) {


        VITIS_LOOP_83_11: for (uint32_t cy = 0; cy < convHeight; ++ cy) {


         VITIS_LOOP_86_12: for (uint32_t cx = 0; cx < convWidth; ++cx) {

          TFXP filterValue = coeff_cache[iChannel][cy][cx];
