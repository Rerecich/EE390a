      ...
      uint32_t convWidth, uint32_t convHeight, uint32_t apply_relu)
{
  ...
  if (logging)
    printf("CConv2DDriver::Conv2D_HW(Input=0x%08X, Output=0x%08X, Coeffes=0x%08X, Biases=0x%08X"
          "NumChannels=%u, NumFilters=%u, Width=%u, Height=%u, FilterSize=%ux%u, ApplyRelu=%u)\n", 
          (uint32_t)input, (uint32_t)output, (uint32_t)coeffs, (uint32_t)biases,
          numChannels, numFilters, inputWidth, inputHeight, convWidth, convHeight, apply_relu);
