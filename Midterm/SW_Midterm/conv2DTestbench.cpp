...
void Conv2D(TFXP *input, TFXP * output, TFXP * coeffs, TFXP * biases,
...
void InitVectors(TFXP * input, TFXP * inputHW, uint32_t sizeInput, 
                TFXP * coeffs, TFXP * coeffsHW, uint32_t sizeCoeffs, TFXP * biases, TFXP * biasesHW, uint32_t sizeBiases)
{
  uint32_t tmp;
...
///////////////////////////////////////////////////////////////////////////////
bool InitDevice(CConv2DDriver & convolver, TFXP* &inputHW, TFXP* &outputHW, TFXP* &coeffsHW, TFXP* &biasesHW, bool log=true)
{
  ...
  biasesHW = (TFXP *)convolver.AllocDMACompatible(600000);
  if (log) {
...
int main(int argc, char ** argv)
{
  ...
  uint32_t errors = 0, error_index = 1;

  CConv2DDriver convolver(true); // Deactivate logging.
  if (!InitDevice(convolver, inputHW, outputHW, coeffsHW, biasesHW, true))
    return -1;
  ...
  InitVectors(input, inputHW, MAX_WIDTH * MAX_HEIGHT * MAX_CHANNELS, coeffs, coeffsHW, MAX_CHANNELS * MAX_FILTERS * 9, biases, biasesHW, 7);
    ...
    */
    ...
    for (uint32_t i = 0; i < currentOutputSize; i++) {
      if(outputHW[i] != outputSW[i]) {
        ...
        if(error_index == 1) {
          error_index = i;
          printf("First mismatch has outputSW = %u and outputHW = %u\n", outputSW[i], outputHW[i]);
        }      
      }
    }
    printf("Number of errors = %u\n", errors);
    printf("First error occurence = %u\n", error_index);
