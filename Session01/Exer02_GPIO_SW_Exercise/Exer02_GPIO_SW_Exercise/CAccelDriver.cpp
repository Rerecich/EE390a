...
CAccelDriver::CAccelDriver(bool Logging)
  ...
  // accelRegs(NULL) - pointer to memory-mapped peripheral registers set to nullptr
  // baseAddr(0) - initial physical base address of accelerator
  // mappingSize(0) - initial size of memory region for peripheral registers
  // logging(Logging) - enables console messages
...
// this is a class destructor, automatically called when an object goes out of scope or is explicitly deleted
// used to clean up resources that class may have acquired during lifetime
CAccelDriver::~CAccelDriver()
{
  ...
  accelRegs = NULL; // to avoid dangling pointers
...
uint32_t CAccelDriver::Open(uint32_t BaseAddr, uint32_t MappingSize, volatile void ** AccelRegsPointer)
{
  ...
  // stores parameters from Open() in the class variables mappingSize and baseAddr
