...
bool LoadFloatWeights(const uint32_t numLayers, float ** weights)
{
  ...
  for (uint32_t iLayer = 0; iLayer < numLayers; ++ iLayer) {
    ...
    uint32_t layerSize = LayerTypes[iLayer] == CONV ? LayerShapes[iLayer][0] * LayerShapes[iLayer][1] * 3*3 : LayerShapes[iLayer][0] * LayerShapes[iLayer][1];
    if ( (weights[iLayer] = (float*)accelDriver.AllocDMACompatible(layerSize * sizeof(float))) == NULL ) {
  ...
  if (weights != NULL)
    accelDriver.FreeDMACompatible(weights);
...
bool ConvertBiasesToFxP(const uint32_t numLayers, float ** floatBiases, TFXP ** fxpBiases)
{
  ...
  if (fxpBiases != NULL)
    accelDriver.FreeDMACompatible(fxpBiases);
...
bool LoadFloatBiases(const uint32_t numLayers, float ** biases)
{
  ...
  for (uint32_t iLayer = 0; iLayer < numLayers; ++ iLayer) {
    ...
    if ( (biases[iLayer] = (float*)accelDriver.AllocDMACompatible(layerSize * sizeof(float))) == NULL ) {
      printf("Error allocating %" PRIu32 " bytes to read file [%s]\n", (uint32_t)(layerSize*sizeof(float)), title);
      fclose(input);
      return false;
    }
  ...
  if (biases != NULL)
    accelDriver.FreeDMACompatible(biases);
